...
export default function PCL5Questionnaire({ navigation }: any) {
  ...
  const [worstEvent, setWorstEvent] = useState<string>('');
  ...
  const getScoreInterpretation = (score: number) => {
    // PCL-5 cutoff score of 33 is recommended for probable PTSD diagnosis
    if (score < 31) {
      return {
        level: 'Low PTSD Symptoms',
        color: '#10b981',
        description: 'Your symptoms suggest minimal PTSD symptoms.',
      };
    } else if (score < 33) {
      return {
        level: 'Moderate PTSD Symptoms',
        color: '#f59e0b',
        description: 'You are experiencing some PTSD symptoms. Consider monitoring your symptoms.',
      };
    } else {
      return {
        level: 'Probable PTSD',
        color: '#ef4444',
        description: 'Your score suggests probable PTSD. It is strongly recommended to consult with a mental health professional for a comprehensive evaluation.',
      };
    }
  };

  const isComplete = () => {
    return Object.keys(responses).length === PCL5_ITEMS.length;
  };
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.aboutCard}>
          ...
          <Text style={styles.aboutText}>
            ...
            Keeping your worst event in mind, please read each problem carefully and indicate how much you 
            have been bothered by that problem in the <Text style={styles.boldText}>past month</Text>.
          </Text>
        </View>
        ...
        <View style={styles.progressCard}>
          <Text style={styles.progressText}>
            Progress: {Object.keys(responses).length} / {PCL5_ITEMS.length}
          </Text>
          <View style={styles.progressBar}>
            <View 
              style={[
                styles.progressFill, 
                { width: `${(Object.keys(responses).length / PCL5_ITEMS.length) * 100}%` }
              ]} 
            />
          </View>
        </View>
        ...
        <View style={styles.scoringCard}>
          ...
          <View style={styles.scoringItem}>
            ...
            <Text style={styles.scoringText}>0-30: Minimal symptoms</Text>
          </View>
          <View style={styles.scoringItem}>
            <View style={[styles.scoringDot, { backgroundColor: '#f59e0b' }]} />
            <Text style={styles.scoringText}>31-32: Moderate symptoms</Text>
          </View>
          <View style={styles.scoringItem}>
            <View style={[styles.scoringDot, { backgroundColor: '#ef4444' }]} />
            <Text style={styles.scoringText}>33+: Probable PTSD (cutoff score)</Text>
          </View>
          <Text style={styles.scoringNote}>
            Note: A score of 33 or higher suggests probable PTSD. Professional evaluation is recommended.
          </Text>
        </View>

        <View style={styles.referenceCard}>
          <Text style={styles.referenceText}>
            Reference: Weathers, F. W., Litz, B. T., Keane, T. M., Palmieri, P. A., Marx, B. P., & 
