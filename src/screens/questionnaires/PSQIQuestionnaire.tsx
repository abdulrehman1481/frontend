...
export default function PSQIQuestionnaire({ navigation }: any) {
  ...
  const [q5a, setQ5a] = useState<number | null>(null); // Cannot sleep within 30 min
  const [q5b, setQ5b] = useState<number | null>(null); // Wake up in middle
  const [q5c, setQ5c] = useState<number | null>(null); // Bathroom
  const [q5d, setQ5d] = useState<number | null>(null); // Cannot breathe
  const [q5e, setQ5e] = useState<number | null>(null); // Cough or snore
  const [q5f, setQ5f] = useState<number | null>(null); // Feel cold
  const [q5g, setQ5g] = useState<number | null>(null); // Feel hot
  const [q5h, setQ5h] = useState<number | null>(null); // Bad dreams
  const [q5i, setQ5i] = useState<number | null>(null); // Pain
  const [q5jText, setQ5jText] = useState(''); // Other reason
  const [q5j, setQ5j] = useState<number | null>(null); // Other frequency

  // Other questions
  const [q6, setQ6] = useState<number | null>(null); // Sleep quality
  const [q7, setQ7] = useState<number | null>(null); // Sleep medicine
  const [q8, setQ8] = useState<number | null>(null); // Trouble staying awake
  const [q9, setQ9] = useState<number | null>(null); // Enthusiasm

  // Question 10
  const [q10, setQ10] = useState<number | null>(null); // Bed partner
  const [q10a, setQ10a] = useState<number | null>(null); // Snoring
  const [q10b, setQ10b] = useState<number | null>(null); // Pauses
  const [q10c, setQ10c] = useState<number | null>(null); // Legs twitching
  const [q10d, setQ10d] = useState<number | null>(null); // Disorientation
  const [q10eText, setQ10eText] = useState(''); // Other restlessness
  const [q10e, setQ10e] = useState<number | null>(null); // Other frequency
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.aboutCard}>
          ...
          <Text style={styles.aboutText}>
            ...
            <Text style={styles.boldText}>past month only</Text>. Your answers should indicate 
            the most accurate reply for the majority of days and nights in the past month.
          </Text>
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.otherReasonGroup}>
            ...
            <TextInput
              ...
              value={q5jText}
              onChangeText={setQ5jText}
              placeholder="Describe any other reasons..."
              multiline
              numberOfLines={2}
              placeholderTextColor="#9ca3af"
            />
                ...
                <View style={styles.optionsGrid}>
                  {FREQUENCY_OPTIONS.map((option) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.optionButton,
                        q5j === option.value && styles.optionButtonSelected,
                      ]}
                      onPress={() => setQ5j(option.value)}>
                      <View style={[
                        styles.radio,
                        q5j === option.value && styles.radioSelected
                      ]}>
                        {q5j === option.value && <View style={styles.radioDot} />}
                      </View>
                      <Text style={[
                        styles.optionLabel,
                        q5j === option.value && styles.optionLabelSelected
                      ]}>
                        {option.label}
                      </Text>
                    </TouchableOpacity>
          ...
          <View style={styles.optionsGrid}>
            {QUALITY_OPTIONS.map((option) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionButton,
                  q6 === option.value && styles.optionButtonSelected,
                ]}
                onPress={() => setQ6(option.value)}>
                <View style={[
                  styles.radio,
                  q6 === option.value && styles.radioSelected
                ]}>
                  {q6 === option.value && <View style={styles.radioDot} />}
                </View>
                <Text style={[
                  styles.optionLabel,
                  q6 === option.value && styles.optionLabelSelected
                ]}>
                  {option.label}
                </Text>
              </TouchableOpacity>
        ...
        {/* Question 7 - Sleep Medicine */}
        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>7. How often have you taken medicine to help you sleep (prescribed or "over the counter")?</Text>
          <View style={styles.optionsGrid}>
            {FREQUENCY_OPTIONS.map((option) => (
              <TouchableOpacity
                key={option.value}
                style={[
                  styles.optionButton,
                  q7 === option.value && styles.optionButtonSelected,
                ]}
                onPress={() => setQ7(option.value)}>
                <View style={[
                  styles.radio,
                  q7 === option.value && styles.radioSelected
                ]}>
                  {q7 === option.value && <View style={styles.radioDot} />}
                </View>
                <Text style={[
                  styles.optionLabel,
                  q7 === option.value && styles.optionLabelSelected
                ]}>
                  {option.label}
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.optionsGrid}>
            {FREQUENCY_OPTIONS.map((option) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionButton,
                  q8 === option.value && styles.optionButtonSelected,
                ]}
                onPress={() => setQ8(option.value)}>
                <View style={[
                  styles.radio,
                  q8 === option.value && styles.radioSelected
                ]}>
                  {q8 === option.value && <View style={styles.radioDot} />}
                </View>
                <Text style={[
                  styles.optionLabel,
                  q8 === option.value && styles.optionLabelSelected
                ]}>
                  {option.label}
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.optionsGrid}>
            {ENTHUSIASM_OPTIONS.map((option) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionButton,
                  q9 === option.value && styles.optionButtonSelected,
                ]}
                onPress={() => setQ9(option.value)}>
                <View style={[
                  styles.radio,
                  q9 === option.value && styles.radioSelected
                ]}>
                  {q9 === option.value && <View style={styles.radioDot} />}
                </View>
                <Text style={[
                  styles.optionLabel,
                  q9 === option.value && styles.optionLabelSelected
                ]}>
                  {option.label}
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.optionsGrid}>
            {BED_PARTNER_OPTIONS.map((option) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionButton,
                  q10 === option.value && styles.optionButtonSelected,
                ]}
                onPress={() => setQ10(option.value)}>
                <View style={[
                  styles.radio,
                  q10 === option.value && styles.radioSelected
                ]}>
                  {q10 === option.value && <View style={styles.radioDot} />}
                </View>
                <Text style={[
                  styles.optionLabel,
                  q10 === option.value && styles.optionLabelSelected
                ]}>
                  {option.label}
                </Text>
              </TouchableOpacity>
            ...
            <View style={styles.partnerQuestionsCard}>
              ...
              <View style={styles.otherReasonGroup}>
                ...
                <TextInput
                  style={styles.textAreaInput}
                  value={q10eText}
                  onChangeText={setQ10eText}
                  placeholder="Describe any other restlessness..."
                  multiline
                  numberOfLines={2}
                  placeholderTextColor="#9ca3af"
                />
                    ...
                    <View style={styles.optionsGrid}>
                      {FREQUENCY_OPTIONS.map((option) => (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.optionButton,
                            q10e === option.value && styles.optionButtonSelected,
                          ]}
                          onPress={() => setQ10e(option.value)}>
                          <View style={[
                            styles.radio,
                            q10e === option.value && styles.radioSelected
                          ]}>
                            {q10e === option.value && <View style={styles.radioDot} />}
                          </View>
                          <Text style={[
                            styles.optionLabel,
                            q10e === option.value && styles.optionLabelSelected
                          ]}>
                            {option.label}
                          </Text>
                        </TouchableOpacity>
        ...
        <View style={styles.referenceCard}>
          <Text style={styles.referenceText}>
            © 1989, University of Pittsburgh. Developed by Buysse, D.J., Reynolds, C.F., Monk, T.H., 
            Berman, S.R., and Kupfer, D.J. Buysse DJ, Reynolds CF, Monk TH, Berman SR, Kupfer DJ: 
...
// Helper component for frequency questions
